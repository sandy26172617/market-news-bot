...
def gemini(prompt):
    models = ["gemini-2.0-flash", "gemini-3.1-flash-lite", "gemini-1.5-pro"]
    for m in models:
...
def analyze_news(articles):
    ...
    prompt = (
        "You are a senior NSE market analyst. Analyze these headlines for Nifty 50 impact.\n\n"
        "Return ONLY a JSON array. Each object must have:\n"
        "- id: number\n"
        "- impact: HIGH or MEDIUM or LOW\n"
        "- skip: true if irrelevant to Indian or global markets\n"
        "- direction: one of: STRONGLY BULLISH, BULLISH, MEDIUM BULLISH, SIDEWAYS, MEDIUM BEARISH, BEARISH, STRONGLY BEARISH\n"
        "- one_liner: one sentence max 20 words on Nifty impact (e.g. FII selling increased, Nifty bearish pressure likely)\n"
        ...
        "Only include HIGH and MEDIUM impact items where skip is false.\n\n"
        "NEWS:\n" + news_text
    )
...
def get_intraday_picks():
    prompt = (
        "You are an expert NSE intraday trader. Based on current Indian market conditions, "
        "suggest 5 Nifty 50 stocks for intraday trading today.\n\n"
        "Return ONLY a JSON array. Each object must have:\n"
        "- stock: NSE symbol e.g. RELIANCE, HDFCBANK, TCS\n"
        "- action: BUY or SELL\n"
        "- reason: one line max 15 words\n"
        "- target: percentage move expected e.g. +1.2%\n"
        "- stoploss: percentage stoploss e.g. -0.5%\n\n"
        "Pick stocks with strong setups based on recent news, sector momentum, and technicals."
    )
    raw = gemini(prompt)
    if not raw:
        return []
...
def get_morning_briefing():
    prompt = (
        "You are a market analyst giving a morning briefing for Indian traders.\n\n"
        "Provide a morning market briefing. Return ONLY a JSON object with these keys:\n"
        "- us_markets: one line on S&P 500, Nasdaq, Dow overnight\n"
        "- asian_markets: one line on Nikkei, Hang Seng, Shanghai\n"
        "- gift_nifty: one line e.g. Gift Nifty at 24350 indicating 80 pt gap up\n"
        "- crude: one line on Brent crude current level and direction\n"
        "- gold: one line on gold current level and direction\n"
        "- usdinr: one line on USD INR level\n"
        "- india_vix: one line on India VIX reading and what it means\n"
        "- outlook: one line overall Nifty opening summary\n"
...
def send_morning_briefing():
    ...
    if not briefing:
        send_telegram(f"<b>Morning Briefing {ts}</b>\n\nUnable to fetch data. Market opens 9:15 AM IST.")
        return
    d = briefing.get("direction", "FLAT")
    emoji = "🚀" if d == "GAP UP" else ("🔻" if d == "GAP DOWN" else "↔️")
    ...
    msg += "━" * 28 + "\n"
    msg += f"{emoji} <b>Today's Outlook: {d}</b>\n"
...
def send_intraday_picks():
    ...
    for p in picks[:5]:
        ...
        msg += f"{action_emoji} <b>{p.get('stock','—')}</b> — {action_label}\n"
        msg += f"<i>{p.get('reason','—')}</i>\n"
        msg += f"Target: <b>{p.get('target','—')}</b>  |  Stop: <b>{p.get('stoploss','—')}</b>\n\n"
    msg += "<i>For educational purposes only. Do your own research before trading.</i>"
    send_telegram(msg)
...
def send_news_alerts(articles):
    ...
    for a in articles:
        ...
        badge  = "🔥" if a["impact"] == "HIGH" else "•"
        msg   += f"{badge} <b>[{cat}]</b> {emoji} <b>{a['direction']}</b>\n"
